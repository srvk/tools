...
def read_analyses(temp_abs, sad, perc, diar = None, label = 'speech'):
    """ When the model has finished producing its outputs, read all the
        ...
        Input:
            ...
            label:the type of speech that needs to be kept (has to be amongst the
                  different kinds of label provided by the diarization model). This
                  parameter is mandatory when the diar parameter has been provided.
    ...
    for file in annotations:
        ...
        with open(os.path.join(temp_abs, file), 'r') as fin:
            ...
            for line in speech_activity:
                ...
                dur = anno_fields[4]
                noiseme_class = anno_fields[7]

                if (not diar_mode) or (diar_mode and noiseme_class == label):
                    tot_dur += float(dur)
...
def main():
    ...
    parser.add_argument('--diar',
            help='''(Optional) name of the diar tool that will be used to '''
                 ...
                 be on the SAD mode.''')
    parser.add_argument('--label', default='CHI',
                        help='''(Optional) the noiseme class of interest. '''
