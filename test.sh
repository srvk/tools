...
# now test Noisemes
cd $OPENSATDIR
sh runOpenSAT.sh /vagrant/test.wav > /dev/null || echo "noisemes failed!" && exit 1
...
sh run-rttm.sh /vagrant/test2.wav /vagrant/test2.rttm > /dev/null || echo "diartk failed!" && exit 1
